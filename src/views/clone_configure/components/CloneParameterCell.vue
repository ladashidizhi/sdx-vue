<template>
  <div
    ...
    style="overflow-y: auto; /* 根据内容是否超出容器高度自动显示/隐藏垂直滚动条 */
    "
  >
    <a-space style="margin-bottom: 15px;">
      <a-button type="primary" @click="onChange" style="height: 23px;margin-right: 80px;">保存修改</a-button>
      <a-tag color="orange">
        <template #icon>
    ...
    >
      <template #optional="{ rowIndex }">
        <trends-form
          ...
          :text="'选项'"
          :params="cloneList[rowIndex].component !== 1 ? {} : {
                  'switch':{
                    'type': 'round'
                  }
                }"
      ...
      <template #is_hide="{ rowIndex }">
        <a-switch size="small" v-model="cloneList[rowIndex].is_hide" type="line"></a-switch>
      </template>
    </a-table>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async handleChange(_data) {
      ...
      await postCloneConfig(this.cloneList, this.cloneId);
    },
  },
};
</script>
