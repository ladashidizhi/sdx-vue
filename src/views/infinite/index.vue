<template>
  <div style="width: 100%; height: 100%">
    <div class="row" id="siteId">
      <div :class="[drawerWidth > 1200 ? 'site-left' : 'site-left-min', 'glass-soft', 'panel']">
        
<div class="panel-head">
  <div class="panel-title">
    <span class="panel-title-text">站点管理</span>
    <span class="panel-badge">列表</span>
  </div>
  <div class="panel-actions">
    <a-space
           class="panel-actions-space" 
        >
          <a-button
            type="primary"
            shape="circle"
  ...
  <div class="panel-body">
        ...
        <a-table class="domain-table"
          ...
          :loading="loading"
          :pagination="false"
          :style="{ height: codeHeight - 140 + 'px' }"
        >
          ...
          <template #select="{ record }">
            <a-checkbox class="row-check" :model-value="record.key === key" @click.stop="rowClicked(record)" />
          </template>
          <template #domain="{ record }">
            <a-space>
              <a-popover v-if="exptionMsg(record)" title="站点存在异常">
                <a-space
                  :style="{
                    backgroundColor: record.key === key ? '#ffff00' : '',
                  }"
                >
                  <a-button
                    status="danger"
                ...
                <template #content>
                  <p v-if="countDate(record.expireDate) < 30">
                    域名即将到期：{{ onTime(record.expireDate) }}
                  </p>
                  <p v-if="record.warning_reason.length > 0">
              ...
              <div v-else>
                ...
                >
                  <a-button
                    size="mini"
                    style="color: #000; background-color: #ffff00"
                  >
                    <template #icon>
          ...
          <template #cdn="{ record }">
            <div style="width: 100%; justify-content: start; display: flex">
              <a-tag class="status-pill" v-if="record.status <= 3" checkable :checked="true">
                <a-popover
                  v-if="record.status === 3"
                  :title="isTypeStatus(record.status) + '信息'"
                >
                  <span :style="{ color: isTypeStyle(record.status) }">
                    {{ isTypeStatus(record.status) }}
                  </span>
                  <template #content>
                    <p :style="{ color: isTypeStyle(record.status) }">
                      {{ record.point }}
                    </p>
                  </template>
                </a-popover>
          ...
          <template #operation="{ record }">
            <a-space>
              <a-popconfirm
                ...
                @cancel="delCancel"
              >
                <a-tag class="op-icon op-del" checkable :checked="true" @click="openDel" style="font-size: 16px;">
                  <icon-delete />
                </a-tag>
              </a-popconfirm>
              <a-color-picker
      ...
      <div
        :class="[drawerWidth > 1200 ? 'site-right' : 'site-right-min', 'glass-soft', 'panel']"
      ...
      >
        <a-tabs @change="(key) => onChange(key)" lazy-load>
          ...
          <a-tab-pane key="1" :destroy-on-hide="true">
            <template #title> <icon-settings />站点设置 </template>
            <a-empty
              v-if="selectIndex === -1"
              description="请选择站点"
              in-config-provider
          ...
          <a-tab-pane key="4" :destroy-on-hide="true" v-if="selectIndex !== -1">
            <template #title> <icon-apps />访问策略 </template>
            <div style="margin: 0px 10px 0px 10px">
              <a-space direction="vertical" size="medium" fill>
                <a-alert style
                  >点击切换下方单选框将自动保存到服务器,切勿频繁点击切换</a-alert
                >
                <a-space>
                  ...
                  <a-alert v-if="istext" type="success"
                    >当前配置已应用到网站开始生效</a-alert
                  >
                  <a-alert v-if="iserror" type="error"
                    >当前配置保存失败不会生效</a-alert
                  >
                </a-space>
          ...
          <a-tab-pane key="6" :destroy-on-hide="true" v-if="selectIndex !== -1">
            <template #title> <icon-file />默认页面 </template>
            <DefaultCell
              :height="codeHeight"
              :webSiteId="webSiteId"
              :status="false"
          ...
          <a-tab-pane key="7" :destroy-on-hide="true" v-if="selectIndex !== -1">
            <template #title> <icon-exclamation-circle />专家模式 </template>
            <ExpertModelView
              :height="codeHeight"
              :webSiteId="webSiteId"
          ...
          <a-tab-pane key="8" :destroy-on-hide="true" v-if="selectIndex !== -1">
            <template #title> <icon-storage />Vhost配置 </template>
            <div class="friendly-class">
              <NginxView
                :webSiteId="webSiteId"
                :parentHeight="codeHeight"
                :domain="selectDomain"
              />
            </div>
          </a-tab-pane>
...
import Add from "@/views/infinite/add_infinite/index.vue";
import Edit from "@/views/infinite/edit_infinite/index.vue";
import FriendlyCell from "@/views/infinite/components/FriendlyCell.vue";
import AdvertisingView from "@/views/infinite/components/AdvertisingView.vue";
import NginxView from "@/views/infinite/components/NginxView.vue";
import BaiduCell from "@/views/infinite/components/BaiduCell.vue";
import DefaultCell from "@/views/infinite/components/DefaultCell.vue";
import References from "@/views/infinite/components/References.vue";
import AccessTacticsCell from "@/views/infinite/components/AccessTacticsCell.vue";
import ExpertModelView from "@/views/web_site/components/ExpertModelView.vue";
import { IconSearch } from "@arco-design/web-vue/es/icon";
import { SysncsGetLimiter, SysncsPostLimiter } from "@/api/website/add.js";
import {
  InfiniteList,
  DelInfinite,
  getInfiniteID,
  UpdateInfinite,
} from "@/api/infinite/index";
...
export default {
  ...
  methods: {
    ...
    async WebSites() {
      ...
      const response = await UpdateInfinite(webSize);
      this.loadings = false;
      if (response.code === 0) {
    ...
    async delSiteWebOk(id) {
      ...
      const response = await DelInfinite(data);
      if (response.code === 0) {
