...
import EditCodeCell from "@/views/infinite/components/EditCodeCell.vue";
import { GetStatistic, postStatistic } from "@/api/site/index";
import { getInfiniteAds, postInfiniteAds } from "@/api/infinite/index";
...
export default {
  ...
  methods: {
    ...
    async onPostStatistic(input, code) {
      ...
      const response = await postStatistic(data);
      if(response.code ==7){
    ...
    async onGetAds() {
      const response = await getInfiniteAds(this.webSiteId);
      if (response.code == 0) {
    ...
    async onPostAds(input, code) {
      ...
      const response =await postInfiniteAds(data);
      if(response.code ==7){
        this.$message.error(response.msg);
        return;
      }
