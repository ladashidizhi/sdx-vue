      ...
      <div class="menus">
        ...
        <div class="file_new danger_op">
          ...
          >
            ...
            <template #default>删除html文件</template>
          </a-button>
        </div>
  ...
  methods: {
    ...
    async getFileList() {
      ...
      try {
        ...
        // this.path = result.path+"/clone_site"
        this.loading = false;
    ...
    async getWebSitePath() {
      // console.log("231321313");
      try {
        const res = await getWebSitePath(this.host.id);
        if (res.code === 7) {
          return false;
        }

        console.log("-----", res.data + "/clone_site");
        this.newPath = res.data + "/clone_site";
        // return res.data;
      } catch (e) {
        return false;
      }
    ...
    async removeHtml(path, isDir, single = false) {
      ...
      this.$modal.confirm({
        ...
        async onOk() {
          try {
            ...
            console.log(that.host.id);
            res = await delDomainHtml(files.join(","), that.host.id);
