<template>
  <div class="filemanager">
    <div class="background">
      <div class="menus">
        <div class="back_btn" @click="backPath">
          <icon-arrow-left />
        </div>
        ...
        <div class="left">
          <div class="file_new">
            <a-dropdown>
              ...
              <template #content>
                <a-doption @click="visiable.createFolder = true">
                  <template #icon>
                    <icon-font type="icon-weibiaoti-_huabanfuben" :size="18" />
                  </template>
                  <template #default>新建文件夹</template>
                </a-doption>
                <a-doption @click="visiable.createFile = true">
                  <template #icon>
                    <icon-font type="icon-wenbenwenjian" :size="18" />
                  </template>
                  <template #default>新建文件</template>
                </a-doption>
              </template>
            </a-dropdown>
            <a-button type="dashed" @click="opTest('upload')">上传</a-button>
            <a-button type="dashed" @click="opTest('remoteDownload')"
              >远程下载</a-button
            >
          </div>

          <div class="file_new danger_op">
            <a-button
              type="primary"
              status="danger"
              @click="remove"
              :disabled="!formAble.delete"
            >
              <template #icon>
                <icon-delete />
              </template>
            ...
            <a-button
              type="primary"
              status="warning"
              :disabled="!formAble.replace"
              @click="$message.warning('该功能正在开发中...')"
            >
              <template #icon>
                <icon-copy />
              </template>
              <template #default>文本替换</template>
            </a-button>
            <!-- <a-button type="primary" status="success" :disabled="!formAble.download">
                            <template #icon>
                                <icon-download />
                            </template>
          ...
          <div class="file_new op">
            <a-button
              type="outline"
              :disabled="!formAble.copy"
              @click="moveFile('copy')"
              >复制</a-button
            >
            <a-button
              type="outline"
              :disabled="!formAble.move"
              @click="moveFile('cut')"
              >移动</a-button
            >
            <a-button
              type="outline"
              :disabled="!formAble.paste"
              @click="pasteFile"
              >粘贴</a-button
            >
            <a-button
              type="outline"
              :disabled="!formAble.compress"
              @click="compressFile"
              >压缩</a-button
            >
            <!-- <a-button type="outline" :disabled="!formAble.auth">权限</a-button> -->
          </div>

          <div class="file_new local">
            <a-button type="primary" status="success" @click="startSSH">
              <template #icon>
                <icon-desktop />
              </template>
              <template #default>终端</template>
            </a-button>
          </div>
          <div class="file_new local">
            <a-button
            ...
            >
              ...
              <template #default>开启直连</template>
            </a-button>
            <a-button v-else @click="direct(false)">
              <template #icon>
                <icon-desktop />
              </template>
              <template #default>关闭直连</template>
            </a-button>
          </div>
        </div>

        <div class="right">
          <div class="file_new search">
            <a-checkbox v-model="search.containSub" style="margin-right: 15px"
              >子目录</a-checkbox
            >
            <a-input-search
              ...
              :button-props="{
                type: 'secondary',
                style:
                  'background-color: var(--color-fill-2); color: var(--color-text-1);border:1px solid #d9d9d9',
              }"
              @clear="
                () => {
                  this.page.current;
          ...
          >
            <template #name="{ record }">
              <a-dropdown trigger="contextMenu" alignPoint>
                <template #content>
                  <a-doption @click="rename(record.path)">重命名</a-doption>
                  <a-doption @click="copyPath(record.path)">复制路径</a-doption>
                  <a-doption
                    @click="remove(record.path, record.isDir, true)"
                    style="color: red"
                    >删除</a-doption
                  >
                </template>
            ...
            <template #mode="{ record }">
              <a-tooltip content="点击修改权限" position="top">
                <a-link @click="updateFileAuth(record)">{{
                  record.mode
                }}</a-link>
              </a-tooltip>
            </template>
            <template #size="{ record }">
              <div>
                <span
                  v-if="(record.isDir && record.computed) || !record.isDir"
                  >{{ computFileSize(record.size) }}</span
                >
                <a-link
                  type="text"
                  size="mini"
                  @click="computDirSize(record)"
                  v-else
                  >计算</a-link
                >
              </div>
            </template>
            <template #modTime="{ record }">{{
              onTime(record.modTime)
            }}</template>
            <template #operation="{ record }">
              <a-button
                type="text"
                size="mini"
                @click="record.isDir ? enterPath(record) : openfile(record)"
                >打开</a-button
              >
              ...
              <a-button
                type="text"
                size="mini"
                :disabled="fileview.indexOf(record.extension) === -1"
                @click="decompressUnzip(record)"
                >解压</a-button
              >
              <a-dropdown trigger="hover">
                ...
                <template #content>
                  <a-doption @click="rename(record.path)">重命名</a-doption>
                  <a-doption @click="scpFileHost(record.path)">发送</a-doption>
                  <a-doption @click="copyPath(record.path)">复制路径</a-doption>
                  <a-doption
                    @click="remove(record.path, record.isDir, true)"
                    style="color: red"
    ...
    <a-modal
      ...
      ok-cancel="false"
      closable="false"
      v-model:visible="visiable.scpname"
      title="发送文件"
      @ok="scpnameRename"
    >
      <div>
        <a-space>
          <div style="width: 120px">接收服务器：</div>
          <a-select
            placeholder="请选择服务器"
            allow-search
            v-model="PataSite.id"
    ...
    >
      <div style="width: 600px; overflow: hidden">
        <a-form :model="compress" :layout="'horizontal'">
          ...
          <!-- <a-form-item field="format" label="压缩格式">
            <a-radio-group v-model="compress.format">
              <a-radio value="zip">zip</a-radio>
              <a-radio value="tar">tar</a-radio>
              <a-radio value="gz">gz</a-radio>
              <a-radio value="bz2">bz2</a-radio>
              <a-radio value="xz">xz</a-radio>
    ...
    <!-- 解压文件 -->
    <a-modal
      v-model:visible="visiable.compressUnzip"
      title="解压目录"
      width="auto"
      draggable
      @ok="decompress"
...
import { formatDateTimeNew } from "@/store/user";
...
import ParasiteCopy from "@/views/web_site/components/ParasiteFileCopy.vue";
...
export default {
  name: "filemanager",
  components: { FileAuth, CodeView, SshView, SaveFile, IconFont, ParasiteCopy },
  setup() {
    const { host, user } = storeToRefs(useUsersStore());
    const columns = [
      {
        title: "名称",
        slotName: "name",
  ...
  methods: {
    async handleDataUpdate(data) {
      this.scpFrom.remoteFile = data;
    },
    async onHost(id) {
      this.isBool = false;
      // console.log(id, "====");
      // console.log(this.PataSite, "-=-=-=-=");
      this.scpFrom.remoteHostId = this.PataSite.id;
      // this.GetHostList();
      this.PataSite.id = id;
      this.isBool = true;
      // await this.refresh();
    ...
    async GetHostList(info) {
      ...
      if (result.code === 0) {
        // this.hostList = result.data.items.filter(item => item.id != this.host.id);
    ...
    scpFileHost(path) {
      // 文件重命名
      this.scpFrom.hostId = this.host.id;
      this.scpFrom.localFile = path;

      // this.renameParams.path = path;
      // this.renameParams.name = path.split("/").pop();
      this.visiable.scpname = true;
    },
    // scpnameRename() {
    //   this.
    //   console.log("-=-=-=-=", this.scpFrom);
    // },
    confirmRename() {
    ...
    async scpnameRename() {
      ...
      ScpFile(this.request, this.scpFrom)
        .then((res) => {
          if (res.code === 7) {
            this.$message.error(`发送文件失败: ${res.msg}`);
            return;
          }
          this.createdParams.filename = "";
          this.$message.success("发送文件完成");
          this.refresh();
        })
        .catch((err) => {
          this.$message.error(`发送文件失败: ${err}`);
        });
    },
    ...
    async decompressUnzip(record) {
      this.unzipPath = record.path;
      this.unzipExper = record.extension.slice(
        record.extension.indexOf(".") > -1
          ? record.extension.indexOf(".") + 1
          : 0
      );

      this.visiable.compressUnzip = true;
    },
    async decompress() {
      // 解压文件
      try {
    ...
    compressFile() {
      var filename = "";
      // console.log("000:", this.path);
      this.selectedKeys.forEach((i) => {
        // console.log(this.files[i].path);
        filename += "_" + this.files[i].path;
        // this.move.files.push(this.files[i].path);
      });

      this.compress.filename =this.removeLeadingUnderscore(
        filename + "_" + formatDateTimeNew(Math.floor(Date.now())));
      // 打开压缩文件显示框
      this.savepath = this.path;
      this.visiable.compress = true;
    },
    trimSlashFromStart(str) {
      return str.replace(/^\//, "");
    },
    removeLeadingUnderscore(str) {
